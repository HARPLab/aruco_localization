...
ArucoBoard::DictType ArucoBoard::getPredefinedDictionaryFromName(std::string const & name) {
	try {
		return cv::aruco::getPredefinedDictionary(aruco::aruco_dict_lookup.at(name));
...
ArucoBoard ArucoBoard::load_from_param_ns(ros::NodeHandle const & nh, std::string const & name, std::string const & prefix) {
	ArucoBoard board;
...
std::vector<ArucoBoard> ArucoBoard::load_from_params(ros::NodeHandle const & nh) {
